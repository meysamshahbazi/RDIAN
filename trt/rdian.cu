...
__global__ void gpuFillBlobFromRGB_rdian(uchar4* im_rgba, int blob_width, int blob_height, int pitch,float* blob)
{
...
void RDIAN::cudaBlobFromImageRGB(void* img_rgb, float* blob, int pitch)
{
    const dim3 blockDim(8, 8);
    const dim3 gridDim(iDivUp(input_w,blockDim.x), iDivUp(input_h,blockDim.y));

    gpuFillBlobFromRGB_rdian<<<gridDim, blockDim, 0, stream>>>((uchar4*) img_rgb, input_w, input_h, pitch, blob);
...
__global__ void CropAndRGBAToRGBGPU_rdian(uchar4* srcImage, float* dstImage, int width, int height, 
...
void RDIAN::cudaCropAndBlobFromImageRGB(void* img_rgb, float* blob) {
    const dim3 blockDim(8, 8);
    const dim3 gridDim(iDivUp(input_w,blockDim.x), iDivUp(input_h,blockDim.y));

    CropAndRGBAToRGBGPU_rdian<<<gridDim, blockDim, 0, stream>>>((uchar4*) img_rgb, blob, img_w, img_h, win.x, win.y, win.width, win.height);
    cudaDeviceSynchronize();
    cudaError_t res;
    res = cudaGetLastError();
    if(res != cudaSuccess ) 
