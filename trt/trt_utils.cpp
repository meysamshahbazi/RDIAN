...
void parseOnnxModel(const std::string & onnx_path,
                    size_t pool_size,
                    ...
                    std::unique_ptr<nvinfer1::IExecutionContext,TRTDestroy> &context)
{
    ...
    std::unique_ptr<nvinfer1::INetworkDefinition,TRTDestroy> network{builder->createNetworkV2(flag)};
    // then parse network 
    std::unique_ptr<nvonnxparser::IParser,TRTDestroy> parser{nvonnxparser::createParser(*network,logger)};
    // parse from file
    ...
    std::unique_ptr<nvinfer1::IHostMemory,TRTDestroy> serializedModel{builder->buildSerializedNetwork(*network, *config)};
...
void saveEngineFile(const std::string & onnx_path,
...
void parseEngineModel(  const std::string & engine_file_path,
                        std::unique_ptr<nvinfer1::ICudaEngine,TRTDestroy> &engine,
                        std::unique_ptr<nvinfer1::IExecutionContext,TRTDestroy> &context)
{
    Logger logger;
...
std::vector< std::vector<float> > generate_anchor(int total_stride, float scale, std::vector<float> ratios, int score_size)
{
